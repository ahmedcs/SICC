...
while l:
	...
	if float(x[1]) >= start and float(x[1]) < start + cur * interval-0.1 and float(x[1]) <= start + simtime:
		 ...
		 if x[0] == '+':  
			#print(x)
		       for count in range(N , N * 2) :           
			 if send[count-N] == 0 and int(x[2]) == count and int(x[7]) == count and int(x[10]) == 0 and int(x[5]) < 80: 
				  #print("pushing send " + str(count) + "\n")
			          #send.insert(count - N, float(x[1]))
				  send[count-N] = float(x[1])
				  #print("pushing send " + str(count) + "source= " + str(x[2]) + " value=" + str(x[1]) + "\n")
				  total1 = total1 + 1	   
			    
		 
		 elif x[0]=='r' : #and total2 <= N :
			for count in range(N , N * 2 ) :
			    if recv[count-N]==0 and int(x[3]) == (N*2 + 1) and int(x[7]) == count and int(x[10]) == 8761 and int(x[5]) > 200 :	
				#print("pushing recv " + str(count) + "\n")
				#recv.insert(count - N, float(x[1]))
				recv[count-N] = float(x[1])
				#print("pushing recv " + str(count) + "dest= " + str(x[3]) + " value=" + str(x[1]) + "\n")
				total2 = total2 + 1		
 	 

	if (total1 == N or total1 == N-1) and float(x[1]) >= start + cur * interval-0.1 and float(x[1]) <= start + simtime:
	   MYFILE = open("fcomp" + str(cur) + ".tr", "w")
	   for i in range(0, N) :
	      print("total= " + str(total1) + "cur= " + str(cur) + "recv= " + str(recv[i]) + "send =" + str(send[i]) + "\n")
	      if recv[i] > 0:
	      	fcomp=(recv[i] - send[i]) * 1000
	      	MYFILE.write(str(i + N) + " " + str(fcomp) + "\n")
	      else:
		MYFILE.write(str(i + N) + " 2000" + "\n")
	   MYFILE.close(); 
