...
for {set j 0} {$j < 5} {incr j} {
	for {set i 0} {$i < [expr $N/2]} {incr i} {    
	    if {[string compare $sourceAlg "Newreno"] == 0 || [string compare $sourceAlg "DC-TCP-Newreno"] == 0} {
		set itcp([expr $i + $j * $N/2]) [new Agent/TCP/FullTcp/Newreno]
		set isink([expr $i + $j * $N/2]) [new Agent/TCP/FullTcp/Newreno]
		$isink([expr $i + $j * $N/2]) listen
	    }
	    if {[string compare $sourceAlg "Sack"] == 0 || [string compare $sourceAlg "DC-TCP-Sack"] == 0} { 
		set itcp([expr $i + $j * $N/2]) [new Agent/TCP/FullTcp/Sack]
		set isink([expr $i + $j * $N/2]) [new Agent/TCP/FullTcp/Sack]
		$isink([expr $i + $j * $N/2]) listen
	    }
	    
	     if {[string compare $sourceAlg "XCP"] == 0 || [string compare $sourceAlg "DC-TCP-XCP"] == 0} { 
		set itcp([expr $i + $j * $N/2]) [new Agent/TCP/FullTcp/Newreno/XCP]
		set isink([expr $i + $j * $N/2]) [new Agent/TCP/FullTcp/Newreno/XCP]
		$isink([expr $i + $j * $N/2]) listen
	    }

	    $ns attach-agent $n([expr $i + $N/2]) $itcp([expr $i + $j * $N/2])
	    $ns attach-agent $nclient $isink([expr $i + $j * $N/2])
	    
	    $itcp([expr $i + $j * $N/2]) set fid_ [expr $i + $N/2]
	    $isink([expr $i + $j * $N/2]) set fid_ [expr $i + $N/2]
    }          
}
...
for {set j 0} {$j < 5} {incr j} {
	for {set i 0} {$i < [expr $N/2]} {incr i} {
	    set iftp([expr $i + $j * $N/2]) [new Application/FTP]
	    $iftp([expr $i + $j * $N/2]) attach-agent $itcp([expr $i + $j * $N/2])    
	    set totaldrops([expr $i + $N/2]) 0	
	}
}
...
$n2 use-rng $rtg2
$n2 set max_ [expr ( $N / 2)] 
$n2 set min_ [expr -1 ]



proc send_incast {} {	
    global ns intersend sendtime round iftp fsize N simulationTime qpacketSize itcp isink n1 n2 tcptype
    set now [$ns now]
	for {set i 0} { $i < [expr  $N / 2]} { incr i } {
		set indexarr($i) 0
	}
    for {set i 0} { $i < [expr $N / 2]} { incr i } {
		...
		$ns connect $itcp([expr $i + (($round -1) * $N / 2)]) $isink([expr $i + (($round -1) * $N / 2)]) 

		set stime [$n1 value]
	        $ns at [expr  $stime + $now] "$iftp([expr $i + (($round -1) * $N / 2)]) send $fsize" 
		puts "in round $round, server [expr $i + (($round -1) * $N / 2)] send at time [expr  $stime + $now]"
