...
ROOT_DIR=`pwd`
TRACES_DIR=$ROOT_DIR/files/tcp-red
...
#rm -rf $TRACES_DIR/$RESULTS_DIR_PREFIX*
    ...
    cd $SCRIPTS_DIR
    echo ns sim-source.tcl $simT TCP RED $nodes $qsize $psize $minrto $tcpopt $sample 
    ns sim-source.tcl $simT TCP RED $nodes $qsize $psize $minrto $tcpopt $sample #> $TRACES_DIR/sim.log
    errorCode=$?
    if [ "$errorCode" -ne 0 ]
    then
