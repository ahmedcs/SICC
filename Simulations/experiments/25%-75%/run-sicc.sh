...
ROOT_DIR=`pwd`
TRACES_DIR=$ROOT_DIR/files/sicc
...
#rm -rf $TRACES_DIR/$RESULTS_DIR_PREFIX*
    ...
    STARTTIME=$(date +%s)
    RESULTS_DIR=$TRACES_DIR/$RESULTS_DIR_PREFIX-simT$simT-N$nodes-Q$qsize-P$psize-MRTO$minrto-TCP$tcpopt-Sample$sample-fupdate$fupdate-qfact$qfact-$(date +%Y-%m-%d.%H.%M.%S)

    cd $SCRIPTS_DIR
    echo ns sim-source.tcl $simT RTCP DropTail/RWNDSYNQ $nodes $qsize $psize $minrto $tcpopt $sample $fupdate $qfact
    ns sim-source.tcl $simT RTCP DropTail/RWNDSYNQ $nodes $qsize $psize $minrto $tcpopt $sample $fupdate $qfact #> $TRACES_DIR/sim.log
    errorCode=$?
    if [ "$errorCode" -ne 0 ]
    then
...
#./draw_queue.sh 
 ...
 #./draw_utilization.sh
  ./flowcompletion.py out.tr $nodes > flowcomp.log
  #./goodput.py thrfile.tr $(($nodes / 2)) 
 #./draw_fcomp.sh $nodes 
 #./goodput.sh $nodes
