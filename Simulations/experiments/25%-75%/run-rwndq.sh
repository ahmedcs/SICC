...
ROOT_DIR=`pwd`
TRACES_DIR=$ROOT_DIR/files/rwndq
...
#rm -rf $TRACES_DIR/$RESULTS_DIR_PREFIX*
    ...
    cd $SCRIPTS_DIR
    echo ns sim-source.tcl $simT RTCP DropTail/RWNDQ $nodes $qsize $psize $minrto $tcpopt $sample $fupdate $qfact
    ns sim-source.tcl $simT RTCP DropTail/RWNDQ $nodes $qsize $psize $minrto $tcpopt $sample $fupdate $qfact #> $TRACES_DIR/sim.log
    errorCode=$?
    if [ "$errorCode" -ne 0 ]
    then
